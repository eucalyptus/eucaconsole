...
class UsersView(LandingPageView):
    ...
    def user_disable(self):
        ...
        with boto_error_handler(self.request):
            ...
            BaseView.log_request(self.request, _(u"Disabling user {0}").format(user_name))
    ...
    def user_enable(self):
        ...
        with boto_error_handler(self.request):
            user_name = self.request.matchdict.get('name')
            BaseView.log_request(self.request, _(u"Enabling user {0}").format(user_name))
...
class UserView(BaseView):
        ...
        with boto_error_handler(self.request):
            ...
            if users_json:
                ...
                for (name, email) in users.items():
                    BaseView.log_request(self.request, _(u"Creating user {0}").format(name))
                    ...
                    statements = []
                    if random_password == 'y':
                        BaseView.log_request(self.request, _(u"Generating password for user {0}").format(user_name))
                        password = PasswordGeneration.generatePassword()
                        self.conn.create_login_profile(name, password)
                        user_data['password'] = password
                    if access_keys == 'y':
                        BaseView.log_request(self.request, _(u"Creating access keys for user {0}").format(user_name))
                    ...
                    if len(statements) > 0:
                        BaseView.log_request(self.request, _(u"Creating policy user {0}").format(user_name))
                        policy['Statement'] = statements
    ...
    def user_update(self):
        ...
        with boto_error_handler(self.request):
            ...
            BaseView.log_request(self.request, _(u"Updating user {0}").format(user_name))
    ...
    def user_change_password(self):
        ...
        try:
            ...
            BaseView.log_request(self.request, _(u"Change password for user {0}").format(self.user.user_name))
            try:
    ...
    def user_random_password(self):
        ...
        with boto_error_handler(self.request):
            ...
            BaseView.log_request(self.request, _(u"Generating password for user {0}").format(self.user.user_name))
            try:
                # try to fetch login profile.
                self.conn.get_login_profiles(user_name=self.user.user_name)
                # if that worked, update the profile
    ...
    def user_delete_password(self):
        ...
        if not(self.is_csrf_valid()):
            return JSONResponse(status=400, message="missing CSRF token")
        with boto_error_handler(self.request):
            BaseView.log_request(self.request, _(u"Deleting password for user {0}").format(self.user.user_name))
    ...
    def user_genKeys(self):
        ...
        if not(self.is_csrf_valid()):
            return JSONResponse(status=400, message="missing CSRF token")
        with boto_error_handler(self.request):
            BaseView.log_request(self.request, _(u"Creating access keys for user {0}").format(self.user.user_name))
    ...
    def user_delete_key(self):
        ...
        with boto_error_handler(self.request):
            BaseView.log_request(self.request, _(u"Creating access key {0} for user {1}").format(key_id, self.user.user_name))
    ...
    def user_deactivate_key(self):
        ...
        key_id = self.request.matchdict.get('key')
        with boto_error_handler(self.request):
            BaseView.log_request(self.request, _(u"Deactivating access key {0} for user {1}").format(key_id, self.user.user_name))
    ...
    def user_activate_key(self):
        ...
        key_id = self.request.matchdict.get('key')
        with boto_error_handler(self.request):
            BaseView.log_request(self.request, _(u"Activating access key {0} for user {1}").format(key_id, self.user.user_name))
    ...
    def user_add_to_group(self):
        ...
        with boto_error_handler(self.request):
            BaseView.log_request(self.request, _(u"Adding user {0} to group {1}").format(self.user.user_name, group))
    ...
    def user_remove_from_group(self):
        ...
        group = self.request.matchdict.get('group')
        with boto_error_handler(self.request):
            BaseView.log_request(self.request, _(u"Removing user {0} from group {1}").format(self.user.user_name, group))
    ...
    def user_delete(self):
        ...
        if self.user is None:
            raise HTTPNotFound
        with boto_error_handler(self.request):
            BaseView.log_request(self.request, _(u"Deleting user {0}").format(self.user.user_name))
    ...
    def user_update_policy(self):
        ...
        with boto_error_handler(self.request):
            BaseView.log_request(self.request, _(u"Updating policy {0} for user {1}").format(policy, self.user.user_name))
    ...
    def user_delete_policy(self):
        ...
        with boto_error_handler(self.request):
            BaseView.log_request(self.request, _(u"Deleting policy {0} for user {1}").format(policy, self.user.user_name))
        ...
        with boto_error_handler(self.request):
            ...
            for i in range(0, len(policy_list)-1):
                if 'dirty' in policy_list[i].keys():
                    ...
                    BaseView.log_request(self.request, _(u"Updating policy {0} for user {1}").format(policies.policies_name[i], self.user.user_name))
            ...
            if len(new_stmts) > 0:
                ...
                if self.EUCA_DEFAULT_POLICY in policies.policy_names:
                    ...
                    BaseView.log_request(self.request, _(u"Updating policy {0} for user {1}").format(self.EUCA_DEFAULT_POLICY, self.user.user_name))
                    ...
                    BaseView.log_request(self.request, _(u"Creating policy {0} for user {1}").format(self.EUCA_DEFAULT_POLICY, self.user.user_name))
                    new_policy = {}
