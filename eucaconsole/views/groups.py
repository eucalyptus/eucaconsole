...
class GroupView(BaseView):
    ...
    def group_create(self):
        if self.group_form.validate():
            ...
            with boto_error_handler(self.request, location):
                BaseView.log_request(self.request, _(u"Creating group {0}").format(new_group_name))
    ...
    def group_update(self):
        if self.group_update_form.validate():
            ...
            if new_group_name is not None or new_path is not None:
                with boto_error_handler(self.request, location):
                    BaseView.log_request(self.request, _(u"Updating group {0}").format(group_name_param))
    ...
    def group_delete(self):
        ...
        if self.group is None:
            raise HTTPNotFound()
        with boto_error_handler(self.request, location):
            BaseView.log_request(self.request, _(u"Deleting group {0}").format(self.group.group_name))
    ...
    def group_add_user(self, group_name, user):
        try:
            BaseView.log_request(self.request, _(u"Adding user {0} to group {1}").format(user, group_name))
    ...
    def group_remove_user(self, group_name, user):
        try:
            BaseView.log_request(self.request, _(u"Removing user {0} from group {1}").format(user, group_name))
    ...
    def group_update_policy(self):
        ...
        with boto_error_handler(self.request):
            BaseView.log_request(self.request, _(u"Updating policy {0} for group {1}").format(policy, self.group.group_name))
    ...
    def group_delete_policy(self):
        ...
        policy = self.request.matchdict.get('policy')
        with boto_error_handler(self.request):
            BaseView.log_request(self.request, _(u"Deleting policy {0} for group {1}").format(policy, self.group.group_name))
