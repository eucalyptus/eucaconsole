...
class Guiops(unittest.TestCase):



    def setUp(cls):
        cls.driver = webdriver.Remote(webdriver_url, webdriver.DesiredCapabilities.FIREFOX)
        cls.driver.implicitly_wait(implicit_wait)
        cls.driver.maximize_window()
        cls.driver.get(console_url)

    def euca_login(self, account, username, password):
        self.verify_visible_element_by_xpath(_eucalyptus_log_in_tab_xpath)
        self.click_element_by_xpath(_eucalyptus_log_in_tab_xpath)
        self.send_keys_by_id(_eucalyptus_account_field_id, account)
        self.send_keys_by_id(_eucalyptus_username_field_id,username)
        self.send_keys_by_id(_eucalyptus_password_field_id,password)
        self.click_element_by_id(_eucalyptus_login_button_id)

    def euca_logout(self, account, username):
        self.wait_for_visible("LINK_TEXT",username+"@"+account)
        self.click_element_by_link_text(username+"@"+account)
        self.wait_for_visible("ID", _user_logout_id)
        self.click_element_by_id(_user_logout_id)
