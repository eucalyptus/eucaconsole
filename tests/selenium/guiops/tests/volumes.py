...
class VolumeOperationsSequence(GuiOps):
    ...
    def __init__(self):
        ...
        self.zones = [zone.strip() for zone in options['zones'].split(',')]
        self.zone_one = self.zones[0]
        self.zone_two = self.zones[1] if len(self.zones) > 1 else self.zone_one
    ...
    def volume_ops_test(self):
        ...
        volume1 = self.tester.create_volume_from_view_page(volume1_name, volume_size=2, availability_zone=self.zone_one)
        volume1_id = volume1.get("volume_id")
        instance1 = self.tester.launch_instance_from_dashboard(availability_zone=self.zone_one, timeout_in_seconds=400)
        ...
        volume2 = self.tester.create_volume_from_dashboard(volume_size=3, availability_zone=self.zone_two)
    ...
    def sortable_volumes_tables_test(self):
        volume1_name = "!!!!"+self.id_generator()+"-volume"
        volume1 = self.tester.create_volume_from_view_page(volume1_name, volume_size=1, availability_zone=self.zone_one)
        volume1_id = volume1.get("volume_id")
        volume2_name = "~~~~"+self.id_generator()+"-volume"
        volume2 = self.tester.create_volume_from_dashboard(volume2_name, volume_size=1, availability_zone=self.zone_one)
        volume2_id = volume2.get("volume_id")
